...
function! WindowOpenProjectExplorerBuffer() "{{{1
  let bufNum = bufnr("__ProjectExplorer__")
  if bufNum == -1
    exe "keepalt edit __ProjectExplorer__"
    return
  endif
  ...
  exe "keepalt buffer __ProjectExplorer__"
endfunction
...
function! project#windowOpen() "{{{1
  ...
  call WindowOpenProjectExplorerBuffer()
  call s:ProjectPrint()
endfunction
